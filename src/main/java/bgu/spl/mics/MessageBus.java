 ...
 * @inv for each m in serviceToQueue.keySet(), serviceToQueue.get(m) != null
 * @inv for each Class<? extends Message> type in messageToService.keySet(), messageToService.get(type) != null
 * @inv for each MicroService m, if m is subscribed to an event or broadcast type, then m should appear in the corresponding message queue or list for that type.
 */
public interface MessageBus {
     ...
     * @param type 	The type to subscribe to.
     * @param m    	The subscribing micro-service.
     * @pre messageToService.get(b.getClass()).contains(m) == false.
     * @post messageToService.get(b.getClass())!= null.
     * @post messageToService.get(b.getClass()).contains(m) == true. 
     * @post messageToService.get(b.getClass()).size() = @pre(messageToService.get(b.getClass()).size()) +1  
     */
     ...
     * @pre serviceToQueue.containsKey(m)==false
     * @post serviceToQueue.containsKey(m) == true
     * @post serviceToQueue.get(m)!=null
     * @post serviceToQueue.get(m) instanceof LinkedBlockingDeque;
     */
     ...
     * @pre serviceToQueue.containsKey(m) == true
     * @post serviceToQueue.containsKey(m) == false
     * @post serviceToQueue.get(m) == null
     * @post for(ConcurrentLinkedQueue<MicroService> queue: messageToService.values()){
	 *		    queue.contains(m)==false
                if(@pre(queue.contains(m))) @post(queue.size= @pre(queue.size) -1 )
	 *       }
     */
