...
public abstract class MicroService implements Runnable {
    ...
    private final ConcurrentHashMap<Class<? extends Message>,Callback<? extends Message>> callBack_hashmap;
    ...
    public MicroService(String name) {
        this.name = name;
        callBack_hashmap = new ConcurrentHashMap<>();
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        callBack_hashmap.put(type, callback);
    }
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        return MessageBusImpl.getInstance().sendEvent(e); // If there's no suitable micro-service it returns null
    }
    ...
    public final void run() {
        ...
        while (!terminated) {
            Message m;
            try {
//                System.out.println(this.getName() + " aWaitMessage for a message ");
                m = MessageBusImpl.getInstance().awaitMessage(this);
//                System.out.println(this.getName() + " Received " + m.getClass() + "Message, will call the callback");
                ((Callback<Message>)callBack_hashmap.get(m.getClass())).call(m);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt(); // Make sure if we need to throw exception
            }
        }
    }
